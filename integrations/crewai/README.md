...
Requires `crewai >= 0.80.0`, installed separately from semantica. If `crewai` is not installed, the integration still imports (classes degrade gracefully), but you can't pass the objects to a `Crew`.
